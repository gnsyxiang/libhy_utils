...
static void _module_destroy(_main_context_t **context_pp)
{
    ...
    module_destroy_t module[] = {
        {"socket client",   &context->socket_handle,    HySocketDestroy},
...
static _main_context_t *_module_create(void)
{
    ...
    socket_config.save_config.type = HY_SOCKET_TYPE_CLIENT;

    // note: 增加或删除要同步到module_destroy_t中
    module_create_t module[] = {
        {"log",             &context->log_handle,           &log_config,        (create_t)HyLogCreate,      HyLogDestroy},
        {"signal",          &context->signal_handle,        &signal_config,     (create_t)HySignalCreate,   HySignalDestroy},
        {"socket client",   &context->socket_handle,        &socket_config,     (create_t)HySocketCreate,   HySocketDestroy},
    };
...
int main(int argc, char *argv[])
{
    ...
    HySocketConnect(context->socket_handle, 10);
