 ...
 * 
 * @file    hy_dynamic_lib_demo.c
 * @brief   
 * @author  gnsyxiang <gnsyxiang@163.com>
 * @date    09/10 2023 14:41
 * @version v0.0.1
 * 
 * @since    note
 * @note     note
 ...
 * 
 *     last modified: 09/10 2023 14:41
 */
#include <stdio.h>
...
#define _APP_NAME "hy_dynamic_lib_demo"
...
int main(int argc, char *argv[])
{
    ...
    do {
        ...
        {
            typedef int (*HyCalcAdd_t)(int a, int b);
            void *handle = NULL;
            HyCalcAdd_t calc_add;
            hy_s32_t ret;

            do {
                handle = HyDynamicLibOpen("../lib/libautotools_demo_lib.so");
                if (!handle) {
                    LOGE("HyDynamicLibOpen failed \n");
                    break;
                }

                calc_add = HyDynamicLibLoadSymbol(handle, "HyCalcAdd");
                if (!calc_add) {
                    LOGE("HyDynamicLibLoadSymbol failed \n");
                    break;
                }
