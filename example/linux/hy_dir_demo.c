 ...
 * 
 *     last modified: 09/10 2023 11:29
 */
#include <stdio.h>
...
static void _read_dir_cb(const char *path, const char *name,
                              HyDirType_e type, void *args)
{
    LOGI("path: %s, name: %s, type: %d \n", path, name, type);
}
...
int main(int argc, char *argv[])
{
    ...
    do {
        ...
        hy_s32_t ret = HyDirRead("./", NULL, HY_DIR_RECURSE_OPEN, _read_dir_cb, context);
        if (ret == -1) {
            LOGE("HyDirRead failed \n");
        }
    } while (0);
